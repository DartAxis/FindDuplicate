...
public class MainProgram {
   ...
   /* private static void compareFiles(Map<String, String> filemap, Map<String, List<String>> fileDuplicate) {
        for (Map.Entry<String, String> en : filemap.entrySet()) {
            if(fileDuplicate.containsKey(en.getValue())){
                fileDuplicate.get(en.getValue()).add(en.getKey());
            } else {
                List<String> filelist=new ArrayList<>();
                filelist.add(en.getKey());
                fileDuplicate.put(en.getValue(),filelist);
            }
        }
    }*/
}
